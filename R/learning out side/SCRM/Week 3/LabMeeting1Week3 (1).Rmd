...
dat <- data.frame(
  ...
  y = rnorm(100,10),
  time = rep(1:5, 20),  
  gender = factor(NA, levels = 1:3,
                  labels = c('male', 'female', 'trans')
                  ),
  age = NA,
  treat = factor(NA, levels = 1:3,
...
dat$gender <- sample(1:3, 100, replace = TRUE)[dat$id]
attributes(dat$gender) <- gndr.att

trt.att <- attributes(dat$treat)
dat$treat <- sample(1:3, 100, replace = TRUE)[dat$id]
...
# replace the dots, remove this comment, and set the chunk to evaluate
dat$cage <- dat$age
dat$cage[dat$age >= 18 & dat$age < 35] <- 1
dat$cage[dat$age >= 35 & dat$age < 45] <- 2
dat$cage[dat$age >= 45 & dat$age < 55] <- 3
cut <- cut(dat$age, c(18, 35, 45, 55))
levels(cut) <- c(1,2,3)
dat$cage <- cut
```
...
```{r, label = answ2d, eval = FALSE}
# replace the dots, remove this comment, and set the chunk to evaluate
tabdat1 <- with(dat, table(treat, gender, cage)) 
```
...
```{r, label = answ2f, eval = FALSE}
# replace the dots, remove this comment, and set the chunk to evaluate
sum1st <- apply(tabdat1, 1, sum)
sum2nd <- apply(tabdat1, 2, sum)
sum3rd <- apply(tabdat1, 3, sum)

tb.gnd.pc <- tabdat1 / sum1st * 100
tb.trt.pc <- ...
tb.age.pc <- ...
```
...
```{r, label = answ2g, eval = FALSE}
input.dupl <- interaction(dat$id, dat$time)
new.dat <- dat[.... , ]
new.dat <- ....
new.dat$y.new <- na.omit(c(tapply(X = ..., INDEX = list(..., ...), FUN =  mean)))
```
...
```{r, label= right3e, eval = FALSE}
?outer
result3 <- outer(...., function(0) {.....})# one line code framework example
```
...
```{r, label = E1answer, eval = FALSE}
apply(abs(cmat, 1, order)
```
